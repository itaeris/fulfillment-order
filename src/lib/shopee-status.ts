...
export async function applyLiveShopeeStatuses(orderNumbers: string[]): Promise<number> {
  ...
  const live = await fetchShopeeOrdersByNumbers(config, unique);
  ...
  await updateOrdersFulfillment(SHOPEE_PLATFORMS, patches);
  return patches.length;
}

export async function refreshOpenShopeeStatuses(options?: {
  limit?: number;
  offset?: number;
}): Promise<{
  checked: number;
  ...
  const open = await getOpenOrderNumbersByPlatforms(SHOPEE_PLATFORMS, OPEN_STATUSES);
  const offset = Math.max(0, options?.offset ?? 0);
  const limit = Math.min(Math.max(options?.limit ?? DEFAULT_LIMIT, 1), 50);
  const checked = open.slice(offset, offset + limit);
  const updated = checked.length > 0 ? await applyLiveShopeeStatuses(checked) : 0;
